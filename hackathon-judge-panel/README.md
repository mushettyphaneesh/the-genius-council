<p align="center">
  ...
  <img src="https://img.shields.io/badge/Featherless-Unlimited%20Tokens-E040FB?style=for-the-badge" alt="Featherless">
  <img src="https://img.shields.io/badge/AI%2FML%20API-Smart%20Model-FF6B35?style=for-the-badge" alt="AI/ML API">
  <img src="https://img.shields.io/badge/Headroom-83%25%20Compression-8B5CF6?style=for-the-badge" alt="Headroom">
</p>
...
| # | Agent | Stage | Role | Model | Provider |
|---|-------|-------|------|-------|----------|
| 1 | **IntakeAgent** | 1 | Extracts structured metadata (problem, solution, track, features) from the submission description | Qwen 2.5-72B | Featherless |
| 2 | **RepoAnalyzer** | 1 | Analyzes GitHub repo: tech stack, agent count, file tree, commit history, Band usage verification | Qwen 2.5-72B | Featherless |
| 3 | **FraudDetector** | 1 | Checks for plagiarism, single-day code dumps, fake Band imports, missing setup files | Qwen 2.5-72B | Featherless |
| 4 | **KGBuilderAgent** | 1 | Fuses all Stage 1 outputs into a compressed knowledge graph using Headroom SmartCrusher | — (no LLM) | — |
| 5 | **BusinessJudge** | 2 | Scores business value: market size, ROI, competitive advantage (0–100) | Qwen 2.5-72B | Featherless |
| 6 | **InnovationJudge** | 2 | Scores technical novelty: architecture creativity, multi-agent design (0–100) | Qwen 2.5-72B | Featherless |
| 7 | **BandJudge** | 2 | Scores Band framework integration depth: room usage, message routing, platform tools (0–100) | Qwen 2.5-72B | Featherless |
| 8 | **DemoJudge** | 2 | Scores demo quality: clarity, completeness, working prototype evidence (0–100) | Qwen 2.5-72B | Featherless |
| 9 | **HeadJudge** | 3 | Checks score divergence, triggers debate if needed, arbitrates final weighted score | Llama 3.3-70B | AI/ML API |
...
### Prerequisites
- Python 3.11+
- A [Featherless](https://featherless.ai) API key — unlimited tokens, $25/mo (promo **BOA26** = free month)
- An [AI/ML API](https://aimlapi.com) key — $10 free credits via lablab (Head Judge smart model only)
...
| Tier | Provider | Model | Agents | Cost per Eval |
|------|----------|-------|--------|---------------|
| **Cheap** | Featherless Premium | Qwen 2.5-72B | 8 of 9 agents (Stage 1 + 2) | **$0** (unlimited tokens) |
| **Smart** | AI/ML API | Llama 3.3-70B | Head Judge only (0-1 calls) | **~$0.002** |

### Why This Works
- **Featherless Premium** ($25/mo, promo BOA26 = free): Unlimited tokens on a subscription model. 4 concurrent connections perfectly matches our 4 parallel Stage 2 judges.
- **AI/ML API** ($10 credits): The Head Judge only makes 1 smart-model call per evaluation, and only during debate. At ~2K tokens per arbitration call, $10 ≈ **500+ evaluations**.
- **Headroom Compression**: Reduces context from ~71K to ~12K tokens per evaluation (83% reduction), making every call faster and cheaper.
...
<p align="center">
  ...
  <br>
  Powered by <a href="https://band.ai">Band</a> · <a href="https://featherless.ai">Featherless</a> · <a href="https://aimlapi.com">AI/ML API</a> · <a href="https://github.com/chopratejas/headroom">Headroom</a>
</p>
