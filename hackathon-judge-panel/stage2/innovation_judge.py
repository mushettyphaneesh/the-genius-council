...
DEBATE_PROMPT = """\
You are an innovation judge participating in a hackathon panel debate.
The panel scores have diverged, and the Head Judge has asked you to review the panel's scores and defend or adjust your innovation score.

Original Innovation Score: {original_score}
...
async def judge_innovation_logic(submission_description: str, kg: str) -> dict:
    """Score the submission on innovation using the compressed knowledge graph."""
    llm = get_cheap_llm()
...
class InnovationJudgeAgent(SimpleAdapter[HistoryProvider]):
    """Innovation Judge Agent Adapter for the Band multi-agent room."""
    ...
    async def on_message(
        ...
        *,
        ...
        if has_responded_since(all_msgs, "[Debate Request]", "[Evaluate Submission]"):
            if not has_responded_since(all_msgs, "[Debate Response Innovation]", "[Debate Request]"):
                # Run debate logic
                original_score_data = get_latest_payload_since(all_msgs, "[Score Innovation]", "[Evaluate Submission]") or {}
                original_score = original_score_data.get("innovation_score", 50)
                ...
                await tools.send_message(content=f"[Debate Response Innovation] {json.dumps(debate_res)}")
                return

        # 2. Otherwise, perform initial scoring if we haven't done so yet
        if has_responded_since(all_msgs, "[Score Innovation]", "[Evaluate Submission]"):
            return
        ...
        if fraud.get("abort_evaluation"):
            ...
            await tools.send_message(content=post_score("innovation_judge", result))
            return

        # Run scoring logic
        kg = kg_payload.get("knowledge_graph", "")
        description = submission.get("description", "")
        result = await judge_innovation_logic(description, kg)
...
# Singleton instance
innovation_judge = InnovationJudgeAgent()
